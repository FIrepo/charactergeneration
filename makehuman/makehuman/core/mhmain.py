...
#        self.reactor = None
    ...
    def saveopts(self, filename):
        import loaddesign
        loaddesign.saveopts(filename)


    def savevalues(self, filename):
        import loaddesign
        loaddesign.savevalues(filename)
    ...
    def loadjson(self, jsonfile):
        ...
        for key in settings.keys():
            loaddesign.set_modifier(key, settings[key])
        loaddesign.commit()


    def randomize(self, randoms, symmetry):
        body = 'b' in randoms
        face = 'f' in randoms
        height = 'h' in randoms
        macro = 'm' in randoms
        self.modules["0_modeling_8_random"].randomize(self.selectedHuman, float(symmetry), macro, height, face, body)


    def loadrender(self, renderfile):
        import json
        import loadrender
        settings = json.load(renderfile)
        for key in settings.keys():
            getattr(loadrender, 'mh_'+key)(settings[key])
        loadrender.mh_render()

    def loadexport(self, exportopts, exfile, exformat):
        import json
        import loadexport
        if exportopts is not None:
            settings = json.load(exportopts)
        else:
            settings = dict()
        if exfile is not None:
            loadexport.mh_setup(exformat)
            for key in settings.keys():
                getattr(loadexport, 'mh_'+key)(settings[key])
            if exfile.find('/') == -1:
                    exfile = self.usercwd + '/' + exfile
            loadexport.mh_export(exfile)
    ...
    def startserver(self, portnum):
        ...
        @app.route("/")
        def hello():
            return """<form id=rform action='render' method=post enctype='multipart/form-data'>
            <ul>
                <li><label for=design>Choose design file</label><input name=design id=design type=file>
                <li><label for=render>Choose render file</label><input name=render id=render type=file>
                <li><label for=3dopts>Choose 3dopts file</label><input name=3dopts id=3dopts type=file>
                <li><label for="skipreset">Use previous values</label><input name=skipreset id=skipreset type=checkbox>
                <li><label for="body">Randomize body</label><input name=body id=body type=checkbox>
                <li><label for="face">Randomize face</label><input name=face id=face type=checkbox>
                <li><label for="height">Randomize height</label><input name=height id=height type=checkbox>
                <li><label for="macro">Randomize macro details</label><input name=macro id=macro type=checkbox>
                <li><label for="symmetry">Randomize symmetry (0..1)</label><input name=symmetry id=symmetry size=4 type=text>
                <li><select name=action><option value=render>Render<option value=fbx>FBX</select>
                </ul><input type=submit></form>"""

        @app.route("/render", methods=['POST'])
        def runrender():
            if 'skipreset' not in request.form:
                self.selectedHuman.load('defaults.mhm', True)
            if 'design' in request.files and request.files['design']:
                self.loadjson(request.files['design'])
            if 'symmetry' in request.form and request.form['symmetry'] != '':
                symmetry = float(request.form['symmetry'])
            else:
                symmetry = 1
            randomvs = ''
            if 'body' in request.form:
                print 'b', request.form['body'], 'b'
                randomvs = randomvs + 'b'
            ...
            if 'macro' in request.form:
                randomvs = randomvs + 'm'
            if randomvs != '':
                self.randomize(randomvs, symmetry)
            if request.form['action'] == 'render':
                if 'render' in request.files and request.files['render']:
                    self.loadrender(request.files['render'])
                else:
                    rend = G.app.getTask('Rendering', 'Render')
                    rend.renderButton.click()#.render()
                task = G.app.getTask('Rendering', 'Viewer')
                outfile = tempfile.gettempdir() + '/' + str(time.clock()) + '.png'
                print outfile
                task.path = outfile
                task.image.save(outfile)
                return send_file(outfile)
            elif request.form['action'] == 'fbx':
                f = None
                if '3dopts' in request.files and request.files['3dopts']:
                    f = request.files['3dopts']
                outfile = tempfile.gettempdir() + '/' + str(time.clock()) + '.fbx'
                self.loadexport(f, outfile, 'fbx')
                return send_file(outfile)
            else:
...
#        self.reactor.runReturn()
    ...
    def _processCommandlineArgs(self, beforeLoaded):
        if beforeLoaded:
            ...
            doc = self.args.get('designopts')
            if doc is not None:
                if doc.find('/') == -1:
                    doc = self.usercwd + '/' + doc
                self.saveopts(doc)

            json = self.args.get('loaddesign')
            if json is not None:
                if json.find('/') == -1:
                    json = self.usercwd + '/' + json
                jfile = open(str(json), 'r')
                self.loadjson(jfile)

            randoms = self.args.get('randomize')
            symmetry = self.args.get('randomsym')
            if symmetry is None:
                symmetry = 1
            if randoms is not None:
                self.randomize(randoms, symmetry)

            exportfile = self.args.get('3dfile')
            if exportfile is not None:
                exportopts = self.args.get('ogreexport')
                if exportopts is not None:
                    if exportopts.find('/') == -1:
                        exportopts = self.usercwd + '/' + exportopts
                    exportopts = open(str(exportopts), 'r')
                    self.loadexport(exportopts, exportfile + '.xml', 'ogre')

                exportopts = self.args.get('daeexport')
                if exportopts is not None:
                    if exportopts.find('/') == -1:
                        exportopts = self.usercwd + '/' + exportopts
                    exportopts = open(str(exportopts), 'r')
                    self.loadexport(exportopts, exportfile + '.dae', 'dae')

                exportopts = self.args.get('fbxexport')
                if exportopts is not None:
                    if exportopts.find('/') == -1:
                        exportopts = self.usercwd + '/' + exportopts
                    exportopts = open(str(exportopts), 'r')
                    self.loadexport(exportopts, exportfile + '.fbx', 'fbx')

                exportopts = self.args.get('objexport')
                if exportopts is not None:
                    if exportopts.find('/') == -1:
                        exportopts = self.usercwd + '/' + exportopts
                    exportopts = open(str(exportopts), 'r')
                    self.loadexport(exportopts, exportfile + '.obj', 'obj')

                exportopts = self.args.get('stlexport')
                if exportopts is not None:
                    if exportopts.find('/') == -1:
                        exportopts = self.usercwd + '/' + exportopts
                    exportopts = open(str(exportopts), 'r')
                    self.loadexport(exportopts, exportfile + '.stl', 'stl')

                exportopts = self.args.get('bvhexport')
                if exportopts is not None:
                    if exportopts.find('/') == -1:
                        exportopts = self.usercwd + '/' + exportopts
                    exportopts = open(str(exportopts), 'r')
                    self.loadexport(exportopts, exportfile + '.bvh', 'bvh')

                exportopts = self.args.get('lmexport')
                if exportopts:
                    self.loadexport(None, exportfile + '.lm.png', 'lm')

                exportopts = self.args.get('uvmexport')
                if exportopts:
                    self.loadexport(None, exportfile + '.uvm.png', 'uvm')

            renderopts = self.args.get('loadrender')
            if renderopts is not None:
                if renderopts.find('/') == -1:
                    renderopts = self.usercwd + '/' + renderopts
                renderfile = open(str(renderopts), 'r')
                self.loadrender(renderfile)

            outfile = self.args.get('savevalues')
            if outfile is not None:
                if outfile.find('/') == -1:
                    outfile = self.usercwd + '/' + outfile
                self.savevalues(outfile)


            outfile = self.args.get('output')
            if outfile is not None:
