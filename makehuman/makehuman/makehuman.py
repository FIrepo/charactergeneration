...
def parse_arguments():
    ...
    parser.add_argument("-d", "--loaddesign", action="store", help="load modelling options file")
    parser.add_argument("-x", "--randomize", action="store", help="randomize specified settings m=macro, h=height, f=face, b=body")
    parser.add_argument("-s", "--randomsym", action="store", help="how symmetrical the randomization should be, 1.0=symmetrical, 0.0=random")
    parser.add_argument("-o", "--output", action="store", help="save rendered image as filename")
    parser.add_argument("-j", "--savevalues", action="store", help="save all model settings to file")
    parser.add_argument("-r", "--loadrender", action="store", help="load json rendering settings file")
    parser.add_argument("--daeexport", action="store", help="load export settings for dae format")
    parser.add_argument("--ogreexport", action="store", help="load json export settings for ogre format")
    parser.add_argument("--stlexport", action="store", help="load json export settings for stl format")
    parser.add_argument("--objexport", action="store", help="load json export settings for wavefront format")
    parser.add_argument("--fbxexport", action="store", help="load json export settings for fbx format")
    parser.add_argument("--bvhexport", action="store", help="load json export settings for bvh format")
    parser.add_argument("--uvmexport", action="store_true", help="export UV Map image")
    parser.add_argument("--lmexport", action="store_true", help="export Lightmap image")
    parser.add_argument("-3", "--3dfile", action="store", help="name for 3d file")
    parser.add_argument("--headless", action="store_true", help="don't show the main window")
    parser.add_argument("--runserver", action="store", help="run web server for API control on specified port")
    parser.add_argument("--designopts", action="store", help="save design options documentation to csv")
    if not isRelease():
